...
export function ToolPanel({ title, icon, children, isOpen, onToggle, accentColor, helpKey }: ToolPanelProps) {
  const colors = {
  ...
  return (
    <section className={cn(
      ...
      <div 
        ...
        role="button"
        tabIndex={0}
        className="w-full flex items-center justify-between p-5 text-left hover:bg-dnd-ink/5 transition-colors cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-dnd-gold/50"
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {helpKey && (
              <HelpButton sectionKey={helpKey} size="sm" className="opacity-70 hover:opacity-100" />
            )}
          </div>
        </div>
