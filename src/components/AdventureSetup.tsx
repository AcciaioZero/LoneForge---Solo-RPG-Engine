          ...
          <div className="bg-dnd-parchment/30 border-2 border-dnd-gold/20 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-4">
                <button
                  onClick={() => setRoomCount(Math.floor(Math.random() * (20 - 3 + 1)) + 3)}
                  className="p-1.5 rounded-lg bg-dnd-red/10 text-dnd-red border border-dnd-red/20 hover:bg-dnd-red/20 transition-all group"
                  title="Random Length"
                >
...
export function TravelSelection({ onFinish }: { onFinish: (terrain: string, duration: number, destination: string) => void }) {
  ...
  const biomes = WILDERNESS_BIOMES.map((name) => ({
    id: name,
    icon: <MapIcon className="w-6 h-6" />, // Default icon, could be more specific
    description: `A journey through the ${name.toLowerCase()}.`
  }));
        ...
        <div className="space-y-8">
          ...
          <div className="bg-dnd-parchment/30 border-2 border-dnd-gold/20 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-4">
                <button
                  onClick={() => setDuration(Math.floor(Math.random() * 14) + 1)}
                  className="p-1.5 rounded-lg bg-emerald-600/10 text-emerald-600 border border-emerald-600/20 hover:bg-emerald-600/20 transition-all group"
...
export function CampSelection({ 
  ...
  onRest: (category?: string) => void, 
  onSkip: () => void, 
  lastLog: string,
  currentEncounter?: any, // Using any here for simplicity or import Encounter from types
  onEncounterCheck?: () => void,
  onResolveEncounter?: (success: boolean) => void,
  ...
  return (
    <div className="min-h-screen bg-dnd-parchment flex items-center justify-center p-4 parchment-texture">
      ...
      >
        ...
        {currentEncounter ? (
          <div className="p-6 rounded-2xl bg-white border-2 border-dnd-gold/30 shadow-inner space-y-6 mb-8">
            <div className="flex flex-wrap gap-2">
              {currentEncounter.oracle_suggested && (
              ...
              {currentEncounter.lore_trigger && (
                <span className="px-2 py-0.5 rounded bg-purple-100 text-purple-700 text-[8px] font-black uppercase tracking-widest border border-purple-200 flex items-center gap-1">
                  <Ghost className="w-2.5 h-2.5" /> Lore Trigger: {currentEncounter.lore_trigger}
                </span>
              )}
            </div>

            <div className="font-serif text-lg leading-relaxed text-dnd-ink antialiased">
              {currentEncounter.text}
            </div>

            {currentEncounter.prompt && (
              <div className="p-4 rounded-xl bg-indigo-50 border border-indigo-100 italic text-sm text-indigo-900 shadow-sm">
                <p className="font-bold uppercase tracking-wider text-[10px] mb-1 opacity-60">Prompt</p>
                {currentEncounter.prompt}
              </div>
            )}

            {currentEncounter.skill_check && (
              <div className="p-5 rounded-2xl bg-dnd-parchment/80 border-2 border-dnd-gold/20 space-y-4">
                <div className="flex justify-between items-center">
                  <p className="text-xs uppercase font-black tracking-widest text-dnd-gold">Resistance Required</p>
                  <span className="font-mono text-sm font-bold text-dnd-red">DC {currentEncounter.skill_check.dc} {currentEncounter.skill_check.skill}</span>
                </div>
                <button 
                  onClick={onEncounterCheck}
                  className="w-full py-4 rounded-2xl bg-dnd-red text-white font-black text-xs uppercase tracking-[0.2em] shadow-xl hover:bg-red-800 transition-all flex items-center justify-center gap-3"
                >
                  <Dices className="w-5 h-5" /> Roll to Withstand
                </button>
              </div>
            )}
            ...
            <div className="flex gap-4">
              {!currentEncounter.skill_check && (
                <button 
                  onClick={() => onResolveEncounter?.(true)}
                  className="flex-1 py-3 rounded-xl bg-indigo-600 text-white font-bold text-xs uppercase tracking-widest shadow-lg"
                >
                  Continue Journey
                </button>
              )}
              <button 
                onClick={onDismissEncounter}
                className="px-6 py-3 rounded-xl border-2 border-dnd-gold/20 text-xs font-bold uppercase tracking-widest text-dnd-ink/60 hover:text-dnd-ink hover:border-dnd-gold/40 transition-all"
              >
                Dismiss
              </button>
            </div>
        ...
        <div className="space-y-6">
          {!currentEncounter ? (
            ...
            <button 
              onClick={onSkip}
              className="w-full p-4 bg-emerald-600/10 border-2 border-emerald-600/20 text-emerald-700 rounded-2xl font-display font-black uppercase tracking-widest hover:bg-emerald-600/20 transition-all text-center"
            >
              Resolve as Long Rest anyway
            </button>
          )}
        </div>
      </motion.div>
