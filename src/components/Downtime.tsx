...
interface DowntimeProps {
  ...
  onAddCustomModifier: (label: string, value: number) => void;
...
export const Downtime: React.FC<DowntimeProps> = ({ 
  ...
  const [customModLabel, setCustomModLabel] = useState('');
  const [customModValue, setCustomModValue] = useState<number>(0);
  ...
  const activeQualityModifiers = useMemo(() => {
    ...
    activeDowntime.customModifiers?.forEach(mod => {
      mods.push({ label: mod.label, value: mod.value, isPositive: mod.value > 0 });
    });
  ...
  if (activeDowntime) {
    if (activeDowntime.resolutionOutcome) {
      return (
        <div className="bg-dnd-paper/80 border-2 border-dnd-gold rounded-3xl p-12 text-center space-y-8 shadow-2xl relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-dnd-red via-dnd-gold to-dnd-red opacity-50" />
          
          <div className="space-y-6 relative z-10">
            <div className="w-24 h-24 bg-dnd-gold/20 rounded-full flex items-center justify-center mx-auto mb-6 outline outline-4 outline-dnd-gold/10">
              <Trophy className="w-12 h-12 text-dnd-gold" />
            </div>
            
            <div className="space-y-2">
              <h2 className="text-4xl font-display font-black text-dnd-ink uppercase tracking-tight">Project Complete!</h2>
              <p className="text-sm font-sans font-bold text-dnd-gold uppercase tracking-[0.3em]">Final Resolution</p>
            </div>

            <div className="max-w-md mx-auto bg-white/50 border-2 border-dnd-gold/30 rounded-2xl p-8 shadow-inner backdrop-blur-sm">
              <div className="inline-block px-4 py-1.5 bg-dnd-red text-white text-xs font-black uppercase tracking-widest rounded-full mb-6 shadow-sm">
                Result: {activeDowntime.resolutionOutcome.name}
              </div>
              <p className="text-xl font-serif italic text-dnd-ink/80 leading-relaxed">
                "{activeDowntime.resolutionOutcome.description}"
              </p>
            </div>
            ...
            <div className="pt-8">
              <button
                onClick={onCancel}
                className="group px-12 py-4 bg-dnd-ink text-white font-display font-black uppercase tracking-[0.2em] rounded-xl hover:bg-dnd-red hover:scale-105 transition-all shadow-xl hover:shadow-dnd-red/20 active:scale-95"
              >
                <span className="flex items-center gap-2">
                  Continue Adventure <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </span>
              </button>
            </div>
          </div>

          <div className="absolute -bottom-12 -right-12 w-48 h-48 bg-dnd-gold/5 rounded-full blur-3xl" />
          <div className="absolute -top-12 -left-12 w-48 h-48 bg-dnd-red/5 rounded-full blur-3xl" />
        </div>
      );
    }
          ...
          <div className="space-y-4">
            ...
            <div className="bg-dnd-paper/50 border border-dnd-gold/20 rounded-2xl p-4">
              ...
              <div className="space-y-2">
                ...
                {activeDowntime.customModifiers?.map((mod) => (
                  <div 
                    key={mod.id}
                    className={cn(
                      "flex justify-between items-center p-2 rounded-lg border text-[10px] font-serif",
                      mod.value >= 0 ? "bg-emerald-50/30 border-emerald-200/50" : "bg-red-50/30 border-red-200/50"
                    )}
                  >
                    <div className="flex flex-col">
                      <span className="font-bold text-dnd-ink">{mod.label}</span>
                      <span className="text-[8px] text-dnd-ink/40 uppercase font-sans">Manual Modifier</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className={cn("font-black", mod.value >= 0 ? "text-emerald-600" : "text-red-600")}>
                        {mod.value >= 0 ? '+' : ''}{mod.value}
                      </span>
                      <button 
                        onClick={() => onRemoveCustomModifier(mod.id)}
                ...
                <div className="p-3 bg-white/30 border border-dashed border-dnd-gold/30 rounded-xl mt-4">
                  <p className="text-[8px] font-black uppercase tracking-widest text-dnd-gold mb-2">Add Manual Modifier</p>
                  <div className="space-y-2">
                    <input 
                      type="text"
                      placeholder="Modifier Reason..."
                      value={customModLabel}
                      onChange={(e) => setCustomModLabel(e.target.value)}
                      className="w-full bg-white/50 border border-dnd-gold/10 rounded-lg px-2 py-1.5 text-[10px] font-serif focus:outline-none focus:border-dnd-gold transition-colors"
                    />
                    <div className="flex gap-2">
                      <input 
                        type="number"
                        placeholder="Value (e.g. 2 or -1)"
                        value={customModValue || ''}
                        onChange={(e) => setCustomModValue(parseInt(e.target.value) || 0)}
                        className="w-24 bg-white/50 border border-dnd-gold/10 rounded-lg px-2 py-1.5 text-[10px] font-mono focus:outline-none focus:border-dnd-gold transition-colors"
                      />
                      <button 
                        disabled={!customModLabel}
                        onClick={() => {
                          onAddCustomModifier(customModLabel, customModValue);
                          setCustomModLabel('');
                          setCustomModValue(0);
                        }}
                        className="flex-1 bg-dnd-gold text-white text-[9px] font-black uppercase tracking-widest rounded-lg transition-all hover:bg-dnd-gold/80 disabled:opacity-50"
                      >
                        Add Modifier
                      </button>
                    </div>
      ...
      <div className="flex items-center gap-4 mb-2">
        ...
        <div>
          <div className="flex items-center gap-1.5">
            ...
            <HelpButton sectionKey="downtime" size="sm" />
          </div>
