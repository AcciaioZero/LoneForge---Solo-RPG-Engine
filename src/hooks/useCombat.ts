...
export const useCombat = ({ gameState, setGameState, addLog }: UseCombatProps) => {
  const handleAttack = useCallback((targetId?: string, hitDieCount?: number, hitModifier?: number) => {
    ...
    let attackBonus = 0;
    let damage = '1d6';
    let isPlayer = activeCombatant.type === 'player';
    ...
    if (isPlayer) {
      ...
      const defaultCalculatedBonus = getProficiencyBonus(gameState.character.level) + getModifier(gameState.character.stats.Strength);
      attackBonus = (hitModifier !== undefined && hitModifier !== null) ? hitModifier : defaultCalculatedBonus;
    ...
    const numDice = isPlayer && hitDieCount ? hitDieCount : 1;
    const rolls: number[] = [];
    for (let i = 0; i < numDice; i++) {
      rolls.push(rollDice(20));
    }
    const rollsSum = rolls.reduce((sum, r) => sum + r, 0);
    const total = rollsSum + attackBonus;
    const dmg = parseDamage(damage);

    const modSign = attackBonus >= 0 ? '+' : '';
    if (rolls.length > 1) {
      addLog('combat', `${attackerName} attacks: Rolls [${rolls.join(', ')}] (Sum: ${rollsSum}) ${modSign} ${attackBonus} = ${total}.`);
    } else {
      addLog('combat', `${attackerName} attacks: Roll ${rolls[0]} ${modSign} ${attackBonus} = ${total}.`);
    }
    
    setGameState(prev => ({ ...prev, hasUsedAction: true }));
    
    return { roll: rolls[0], total, damage: dmg };
