...
import { 
  Character, GameState, Enemy, Room, LogEntry, GameContext, Attribute, Skill, Ability, Item, Background, Species, CharacterClass, Spell, SpellSlot, SettlementType, SettlementEvent, Npc, Faction, Quest, OracleResponse, Notification, RoomRolls, Companion, EncounterSuggestion, ItemQuest, ActiveDowntime, DowntimeEvent, Encounter, QuestTone, District, DistrictType, SettlementTheme
} from '../types';
import { 
  INITIAL_CHARACTER, INITIAL_GAME_STATE, ORACLE_LIKELIHOODS, DIFFICULTY_CLASSES, SETTLEMENT_TYPES, SKILL_ATTRIBUTES, CR_VALUES, SETTLEMENT_EVENTS, WILDERNESS_TERRAINS, WILDERNESS_BIOMES, CAMP_DISTURBANCE_CATEGORIES, URBAN_EVENT_CATEGORIES, SITUATION_VERBS,
  DUNGEON_ROOM_ENTRIES, DUNGEON_ROOM_ENTRIES_WITH_DETAILS, DUNGEON_COMPLETION
} from '../constants';
import { BACKGROUND_TEMPLATES, CustomBackgroundState } from '../data/backgrounds';
import { ORIGIN_FEATS } from '../data/origin_feats';
import { Feat } from '../data/feats';
import { 
  generateFaction, generateFactionMotto, generateFactionInfluence, generateFactionHeadquarters, generateFactionLeader, generateFactionGoal, generateFactionHook, generateFactionSecret, generateFactionDescription, generateLoot, rollDice, getModifier, calculateAc, parseDamage, generateRoom, rerollCategory, getOracleResponse, getPrevXpThreshold, getXpRequired, getSkillModifier, getSuggestedRoleForLocation, crToNumber, generateSettlement, getProficiencyBonus,
  generateWildernessEvent, generateWildernessDiscovery, generateCampDisturbance, generateCampDisturbanceByCategory, generateUrbanEvent, generateUrbanEventByCategory, generateDistrictDisturbance, generateAdventure, generateCrack, generateCostOfInaction, generateHiddenTwist, generateOpenQuest, generateDistrictLocations,
...
import { SETTLEMENTS } from '../data/settlement_themes';
...
export function useGameState() {
    ...
    if (saved) {
      try {
        ...
        const character: Character = {
          ...
          spellSaveDcBonus: parsed.character?.spellSaveDcBonus ?? 0,
          spellAttackBonus: parsed.character?.spellAttackBonus ?? 0,
          ...
          spellcastingAbility: parsed.character?.spellcastingAbility,
          powerState: parsed.character?.powerState || INITIAL_CHARACTER.powerState
        };
        ...
        character.spellSaveDcBonus = Number(character.spellSaveDcBonus) || 0;
        character.spellAttackBonus = Number(character.spellAttackBonus) || 0;
        ...
        return {
          ...
          adventureHook: (parsed.adventureHook && !parsed.adventureHook.situation) ? null : (parsed.adventureHook || null)
        };
      } catch (e) {
  ...
  const [isTrapExpanded, setIsTrapExpanded] = useState(false);
  ...
  const [itemTab, setItemTab] = useState<'compendium' | 'generator' | 'cursed'>('compendium');
  const [villainTab, setVillainTab] = useState<'dungeon_boss' | 'local_villain' | 'bbeg'>('dungeon_boss');
  const [generatedFaction, setGeneratedFaction] = useState<Faction | null>(null);
  const [isFactionSecretRevealed, setIsFactionSecretRevealed] = useState(false);
  ...
  const [selectedSkill, setSelectedSkill] = useState<Skill | 'None'>('None');
  ...
  const [combatHitDieCount, setCombatHitDieCount] = useState(1);
  const [combatHitModifier, setCombatHitModifier] = useState<number | null>(null);
  ...
  const handleRestartSameCharacter = useCallback(() => {
    ...
    setActiveTab('adventure');
    addLog('narrative', "By a miracle, you stand once more. You have managed to escape with your life, but you are gravely wounded. Your journey continues from where you fell.");
  }, [addLog]);

  const handleResolveEncounter = useCallback((success: boolean) => {
    const encounter = gameState.currentEncounter;
    if (!encounter) return;

    const outcomeText = success 
      ? `Success: ${encounter.skill_check?.success}` 
      : `Failure: ${encounter.skill_check?.failure}`;
    
    addLog('narrative', outcomeText);
    setGameState(prev => ({ 
      ...prev, 
      lastTravelLog: `${prev.lastTravelLog}\n\nOutcome: ${outcomeText}`,
      currentEncounter: null,
      isCamping: false // Clear camping state on resolution
    }));
  }, [gameState.currentEncounter, addLog]);

  const handleEncounterSkillCheck = useCallback(() => {
    const encounter = gameState.currentEncounter;
    if (!encounter || !encounter.skill_check) return;

    const { skill, dc } = encounter.skill_check;
    ...
    handleResolveEncounter(success);
  }, [gameState.currentEncounter, gameState.character, handleResolveEncounter, addLog]);
  ...
  const handleTravelDay = useCallback(() => {
    ...
    if (hasEvent) {
      encounter = generateWildernessEvent(gameState.travel.terrain);
    }

    setGameState(prev => {
      if (!prev.travel) return prev;
      const log = hasEvent 
        ? `Day ${nextDay}: ${encounter?.text}` 
    ...
    if (hasEvent && encounter) {
      addLog('narrative', `Day ${nextDay}: ${encounter.text}`);
    } else {
  ...
  const handleGenerateWildernessEvent = useCallback(() => {
    if (!gameState.travel) return;
    const encounter = generateWildernessEvent(gameState.travel.terrain);
    if (!encounter) return;
    addLog('narrative', `Wilderness Event: ${encounter.text || 'Something happens in the wilderness.'}`);
    setGameState(prev => ({ 
      ...prev, 
      lastTravelLog: `Event: ${encounter.text || 'Something happens.'}`,
      currentEncounter: encounter
    }));
  }, [gameState.travel, addLog]);
  ...
  const handleGenerateWildernessDiscovery = useCallback(() => {
    if (!gameState.travel) return;
    const encounter = generateWildernessDiscovery(gameState.travel.terrain);
    if (!encounter) return;
    addLog('narrative', `Discovery: ${encounter.text || 'You find something interesting.'}`);
    setGameState(prev => ({ 
      ...prev, 
      lastTravelLog: `Discovery: ${encounter.text || 'You find something.'}`,
      currentEncounter: encounter
    }));
  }, [gameState.travel, addLog]);
  ...
  const handleCampRest = useCallback((category?: string) => {
    const encounter = category 
      ? generateCampDisturbanceByCategory(category)
      : generateCampDisturbance();
    
    if (!encounter) return;
    addLog('narrative', `Night Disturbance: ${encounter.text || 'The night is restless.'}`);
    setGameState(prev => ({ 
      ...prev, 
      isDayActive: false,
      lastTravelLog: `Night: ${encounter.text || 'Restless night.'}`,
      currentEncounter: encounter
    }));
  }, [addLog]);
  ...
  const handleSettlementInteraction = useCallback((interaction: string, locationName: string, locationCategory: string) => {
    ...
    if (response.type === 'narrative') {
      ...
      if (Number(gameState.character.gold) >= cost) {
        setGameState(prev => {
          const newChar = { ...prev.character, gold: Number(prev.character.gold) - cost };
  ...
  const handleExplore = useCallback(() => {
    ...
    // Decide whether to use a generic entry or one with details (70/30 split)
    const useDetailed = Math.random() < 0.3; // 30% chance for detailed as requested (user said 70/30, usually means 70% generic 30% detailed in this context)
    
    const pickRandom = <T>(arr: T[]): T => arr[rollDice(arr.length) - 1];

    const entryMessage = useDetailed
      ? pickRandom(DUNGEON_ROOM_ENTRIES_WITH_DETAILS)(newRoom.type, newRoom.feature)
      : pickRandom(DUNGEON_ROOM_ENTRIES);

    addLog('narrative', entryMessage);
    
    let roomNarrative = entryMessage;

    if (newRoom.trap) {
      addLog('narrative', `DANGER! ${newRoom.trap}`);
    } else if (newRoom.environmentFeature) {
      addLog('narrative', `Environment: ${newRoom.environmentFeature}`);
    }

    if (newRoom.clue) {
      addLog('narrative', `Clue: ${newRoom.clue.text}`);
      addLog('narrative', `Implication: ${newRoom.clue.implication}`);
      if (newRoom.clue.lore_trigger) {
        addLog('narrative', newRoom.clue.lore_trigger);
      }
    }

    if (newRoom.event) {
      addLog('narrative', `Event: ${newRoom.event}`);
    }

    if (newRoom.encounterDifficulty) {
      addLog('narrative', `A threat reveals itself: ${newRoom.encounterDifficulty} challenge. Use the encounter suggester in the Combat tool section to generate the encounter `);
    }

    if (newRoom.npcIntro) {
      addLog('narrative', newRoom.npcIntro.text);
      if (newRoom.npcIntro.context) {
        addLog('narrative', newRoom.npcIntro.context);
      }
    }

    if (isComplete) {
      const msg = pickRandom(DUNGEON_COMPLETION);
      addLog('narrative', msg);
      roomNarrative += "\n" + msg;
    }

    setGameState(prev => ({ ...prev, lastRoomLog: roomNarrative }));
  ...
  const handleUnlockPower = useCallback((powerId: string) => {
    setGameState(prev => {
      const currentPowerState = prev.character.powerState || INITIAL_CHARACTER.powerState!;
      if (currentPowerState.unlockedPowerIds.includes(powerId)) return prev;
      ...
      return {
        ...
        character: {
          ...
          powerState: {
            ...currentPowerState,
            unlockedPowerIds: [...currentPowerState.unlockedPowerIds, powerId]
          }
        }
      };
    });
  ...
  const handleUpdatePowerMeter = useCallback((meterName: string, amount: number) => {
    setGameState(prev => {
      const currentPowerState = prev.character.powerState || INITIAL_CHARACTER.powerState!;
      const current = currentPowerState.meters[meterName] || 0;
  ...
  const handleCompletePowerTrial = useCallback((trialId: string) => {
    setGameState(prev => {
      const currentPowerState = prev.character.powerState || INITIAL_CHARACTER.powerState!;
      if (currentPowerState.completedTrialIds.includes(trialId)) return prev;
      ...
      return {
        ...
        character: {
          ...
          powerState: {
            ...currentPowerState,
            completedTrialIds: [...currentPowerState.completedTrialIds, trialId]
          }
        }
      };
    });
  ...
  const handleManifestationRoll = useCallback((tableId: string) => {
    // This will be handled in the UI/Engine call
  }, []);
  ...
  const handleUpdateSpellcastingAbility = useCallback((ability: Attribute) => {
    setGameState(prev => ({
      ...prev,
      character: {
        ...prev.character,
    ...
    addLog('narrative', `Spellcasting ability updated to ${ability}.`);
  }, [addLog]);
  ...
  const handleGenerateQuest = useCallback((tone?: QuestTone) => {
    const quest = generateAdventure(tone);
    setGameState(prev => ({ ...prev, adventureHook: quest }));
    setIsQuestModalOpen(true);
    addLog('narrative', `New Quest Generated! Tone: ${(quest.situation?.tone || 'Adventure').toUpperCase()}`);
  }, [addLog]);

  const handleGenerateOpenQuest = useCallback(() => {
    const incipit = generateOpenQuest();
    const questNote = `\n\n--- Open Quest Hook ---\n${incipit}`;
    ...
    addLog('narrative', "New Open Quest generated and added to your notes!");
    addLog('narrative', incipit);
  }, [addLog]);

  const handleAddFeat = useCallback((feat: Feat) => {
    setGameState(prev => {
      const char = prev.character;
      const newAbility: Ability = {
        id: `feat-${feat.name.toLowerCase().replace(/\s+/g, '-')}`,
        name: feat.name,
        description: feat.description,
        type: 'passive', // Feats are usually passive unless they grant actions
        // In a more complex system, we could parse effect types to set 'action' etc.
      };

      // Check if already added
      if (char.customAbilities?.some(a => a.name === feat.name)) {
        return prev;
      }
      ...
      return {
        ...
        character: {
          ...char,
          customAbilities: [...(char.customAbilities || []), newAbility]
        }
      };
    });
    addLog('narrative', `Added feat: ${feat.name}`);
  }, [addLog]);

  const handleUpdateSettlementName = useCallback((newName: string) => {
    setGameState(prev => {
      if (!prev.currentSettlement) return prev;
      return {
        ...prev,
        currentSettlement: { ...prev.currentSettlement, name: newName }
      };
    });
    addLog('narrative', `Settlement renamed to: ${newName}`);
  }, [addLog]);

  const handleImportSave = useCallback((newData: any) => {
    // Basic validation: must have character
    if (!newData || typeof newData !== 'object') {
      console.error('Import Error: Data is null or not an object');
      return false;
    }
    
    if (!newData.character) {
      console.error('Import Error: Missing character property', newData);
      return false;
    }

    // Use a robust merging similar to the initial load to ensure all required fields are present
    const character: Character = {
      ...INITIAL_CHARACTER,
      ...newData.character,
      stats: {
        ...INITIAL_CHARACTER.stats,
        ...(newData.character?.stats || {})
      },
      inventory: Array.isArray(newData.character?.inventory) ? newData.character.inventory : [],
      treasure: Array.isArray(newData.character?.treasure) ? newData.character.treasure : [],
      abilities: Array.isArray(newData.character?.abilities) ? newData.character.abilities : INITIAL_CHARACTER.abilities,
      customAbilities: Array.isArray(newData.character?.customAbilities) ? newData.character.customAbilities : [],
      companions: Array.isArray(newData.character?.companions) ? newData.character.companions : [],
    };

    // Ensure numeric values are numbers
    character.hp = Number(character.hp) || character.maxHp || 10;
    character.maxHp = Number(character.maxHp) || 10;
    character.level = Number(character.level) || 1;
    character.gold = Number(character.gold) || 0;
    character.xp = Number(character.xp) || 0;
    ...
    const validatedData: GameState = {
      ...
      notifications: Array.isArray(newData.notifications) ? newData.notifications : [],
      context: newData.context || 'Narrative',
    };

    setGameState(validatedData);
    addLog('system', 'Game state imported successfully');
    return true;
  }, [addLog, setGameState]);

  const handleUpdateDistrictName = useCallback((districtId: string, newName: string) => {
    setGameState(prev => {
      if (!prev.currentSettlement) return prev;
      return {
        ...prev,
        currentSettlement: {
          ...
          districts: prev.currentSettlement.districts.map(d => 
            d.id === districtId ? { ...d, name: newName } : d
          )
        }
      };
    });
    addLog('narrative', `District renamed to: ${newName}`);
  }, [addLog]);

  const handleUpdateLocationName = useCallback((locationId: string, newName: string) => {
    setGameState(prev => {
      if (!prev.currentSettlement) return prev;
      return {
        ...prev,
    ...
    addLog('narrative', `Location renamed to: ${newName}`);
  }, [addLog]);

  const handleRerollDistrictLocations = useCallback((districtId: string) => {
    setGameState(prev => {
      if (!prev.currentSettlement) return prev;
      const district = prev.currentSettlement.districts.find(d => d.id === districtId);
      if (!district) return prev;

      const newLocations = generateDistrictLocations(
        district.type, 
        prev.currentSettlement.name, 
        district.id, 
        prev.character.level,
        prev.currentSettlement.type
      );
      ...
      return {
        ...
        currentSettlement: {
          ...prev.currentSettlement,
          districts: prev.currentSettlement.districts.map(d => 
            d.id === districtId ? { ...d, locations: newLocations } : d
          )
        }
      };
    });
    addLog('narrative', `District buildings rerolled.`);
  }, [addLog]);

  const handleAddDistrict = useCallback((type: DistrictType) => {
    setGameState(prev => {
      if (!prev.currentSettlement) return prev;
      
      const newDistrictId = `district-${Date.now()}`;
      const locations = generateDistrictLocations(
        type,
        prev.currentSettlement.name,
        newDistrictId,
        prev.character.level,
      ...
      const newDistrict: District = {
        ...
        name: `${type} District`,
        type,
        description: `A district focused on ${type.toLowerCase()} activities.`,
        locations
      };
      ...
      return {
        ...
        currentSettlement: {
          ...prev.currentSettlement,
          districts: [...prev.currentSettlement.districts, newDistrict]
        }
      };
    });
    addLog('narrative', `New ${type} district established in ${gameState.currentSettlement?.name || 'the settlement'}.`);
  }, [addLog, gameState.currentSettlement?.name]);

  const handleRemoveDistrict = useCallback((districtId: string) => {
    setGameState(prev => {
      if (!prev.currentSettlement) return prev;
      
      const districtToRemove = prev.currentSettlement.districts.find(d => d.id === districtId);
      if (!districtToRemove) return prev;

      return {
        ...prev,
        currentSettlement: {
          ...prev.currentSettlement,
          districts: prev.currentSettlement.districts.filter(d => d.id !== districtId)
        }
      };
    });
    addLog('narrative', `District removed from the settlement.`);
  }, [addLog]);

  const handleGenerateSettlementTheme = useCallback(() => {
    setGameState(prev => {
      if (!prev.currentSettlement) return prev;
      
      const settlementTypeKey = prev.currentSettlement.type.toUpperCase();
      const settlementLists = SETTLEMENTS[settlementTypeKey];
      
      if (!settlementLists) return prev;

      const pickRandom = (arr?: any[]) => arr && arr.length > 0 ? arr[Math.floor(Math.random() * arr.length)] : 'None';

      const theme = {
        biome: pickRandom(settlementLists.biome_tags),
        aesthetic: pickRandom(settlementLists.aesthetic),
        government: pickRandom(settlementLists.government),
        government_alignment: pickRandom(settlementLists.government_alignment),
        population: pickRandom(settlementLists.population),
        economy: pickRandom(settlementLists.economy),
        tensions: pickRandom(settlementLists.tensions),
        religion: pickRandom(settlementLists.religion),
        sensory_details: pickRandom(settlementLists.sensory_details),
        landmark_or_centerpiece: pickRandom(settlementLists.landmark_or_centerpiece)
      };
    ...
    addLog('narrative', `Settlement theme rolled for ${gameState.currentSettlement?.name}.`);
  }, [addLog, gameState.currentSettlement?.name]);

  const handleRerollSettlementThemeField = useCallback((field: keyof SettlementTheme) => {
    setGameState(prev => {
      if (!prev.currentSettlement || !prev.currentSettlement.theme) return prev;
      ...
      let newValue = '';
      switch (field) {
        case 'biome': newValue = pickRandom(settlementLists.biome_tags); break;
        case 'aesthetic': newValue = pickRandom(settlementLists.aesthetic); break;
        case 'government': newValue = pickRandom(settlementLists.government); break;
        case 'government_alignment': newValue = pickRandom(settlementLists.government_alignment); break;
        case 'population': newValue = pickRandom(settlementLists.population); break;
        case 'economy': newValue = pickRandom(settlementLists.economy); break;
        case 'tensions': newValue = pickRandom(settlementLists.tensions); break;
        case 'religion': newValue = pickRandom(settlementLists.religion); break;
        case 'sensory_details': newValue = pickRandom(settlementLists.sensory_details); break;
        case 'landmark_or_centerpiece': newValue = pickRandom(settlementLists.landmark_or_centerpiece); break;
      }
  ...
  const handleGenerateFaction = useCallback(() => {
    ...
    setIsFactionSecretRevealed(false);
  }, []);

  const handleRerollFactionField = useCallback((field: keyof Faction) => {
    if (!generatedFaction) return;

    setGeneratedFaction(prev => {
      if (!prev) return null;
      const f = { ...prev };
      switch (field) {
        case 'motto':
          f.motto = generateFactionMotto();
          break;
        case 'influence':
          f.influence = generateFactionInfluence();
          break;
        case 'headquarters':
          f.headquarters = generateFactionHeadquarters(f.type);
          break;
        case 'leader':
          f.leader = generateFactionLeader();
          break;
        case 'goal':
          f.goal = generateFactionGoal();
          break;
        case 'hook':
          f.hook = generateFactionHook(f.name, f.motto);
          break;
        case 'secret':
          f.secret = generateFactionSecret(f.name, f.leader, f.type);
          break;
      }
      
      // Update description if it depends on the changed field
      if (field === 'influence' || field === 'goal') {
        f.description = generateFactionDescription(f.name, f.type, f.influence, f.goal);
      }
  ...
  const handleBackgroundSelect = useCallback((background: Background, mode: 'standard' | 'custom' = 'standard') => {
    const template = BACKGROUND_TEMPLATES[background];
    if (!template && mode === 'standard') return;

    setGameState(prev => {
      const newChar = { ...prev.character };
      newChar.background = background;
      newChar.backgroundMode = mode;
      
      if (mode === 'standard' && template) {
        newChar.originFeat = template.origin_feat;
        newChar.proficiencies = [...template.skill_proficiencies];
        newChar.toolProficiencies = template.tool_proficiency ? [template.tool_proficiency] : [];
        newChar.featChoices = {}; // Reset choices when switching template
      } else if (mode === 'custom') {
        if (!newChar.customBackground) {
          newChar.customBackground = {
            stats: { 
  ...
  const handleCustomBackgroundChange = useCallback((update: Partial<CustomBackgroundState>) => {
    setGameState(prev => ({
      ...prev,
      character: {
        ...prev.character,
        customBackground: prev.character.customBackground ? {
          ...prev.character.customBackground,
          ...update
        } : {
          stats: { 
            'Strength': 0, 'Dexterity': 0, 'Constitution': 0, 
  ...
  const handleUpdateSpellSaveDcBonus = useCallback((delta: number) => {
    setGameState(prev => ({
      ...prev,
      character: {
        ...prev.character,
        spellSaveDcBonus: (prev.character.spellSaveDcBonus || 0) + delta
      }
    }));
  }, []);

  const handleUpdateSpellAttackBonus = useCallback((delta: number) => {
    setGameState(prev => ({
      ...prev,
      character: {
        ...prev.character,
        spellAttackBonus: (prev.character.spellAttackBonus || 0) + delta
      }
    }));
  }, []);
  ...
  const handleRerollDie = useCallback((category: string) => {
    ...
    let newNarrative = "";
    if (category === 'red' && newRoom.npcIntro) {
      addLog('narrative', newRoom.npcIntro.text);
      newNarrative += "\n" + newRoom.npcIntro.text;
      if (newRoom.npcIntro.context) {
        addLog('narrative', newRoom.npcIntro.context);
        newNarrative += "\n" + newRoom.npcIntro.context;
      }
    } else if (category === 'green') {
      if (newRoom.trap) {
        const msg = `DANGER! ${newRoom.trap}`;
        addLog('narrative', msg);
        newNarrative += "\n" + msg;
      } else if (newRoom.environmentFeature) {
        const msg = `Environment: ${newRoom.environmentFeature}`;
        addLog('narrative', msg);
        newNarrative += "\n" + msg;
      }
    } else if (category === 'blue' && newRoom.clue) {
      const msg = `Clue: ${newRoom.clue.text}`;
      const implicationMsg = `Implication: ${newRoom.clue.implication}`;
      addLog('narrative', msg);
      addLog('narrative', implicationMsg);
      newNarrative += "\n" + msg + "\n" + implicationMsg;
      if (newRoom.clue.lore_trigger) {
        addLog('narrative', newRoom.clue.lore_trigger);
        newNarrative += "\n" + newRoom.clue.lore_trigger;
      }
    } else if (category === 'multicolour' && newRoom.event) {
      const msg = `Event: ${newRoom.event}`;
      addLog('narrative', msg);
      newNarrative += "\n" + msg;
    } else if (category === 'gold' && newRoom.lootResult) {
      const msg = `Possibility of loot detected.`;
      addLog('narrative', msg);
      newNarrative += "\n" + msg;
    } else if (category === 'purple' && newRoom.encounterDifficulty) {
      const msg = `A threat reveals itself: ${newRoom.encounterDifficulty} challenge. Use the encounter suggester in the Combat tool section to generate the encounter `;
      addLog('narrative', msg);
      newNarrative += "\n" + msg;
    }

  }, [gameState.currentRoom, gameState.character.level, gameState.dungeonConfig, gameState.lastRoomLog, addLog]);
  ...
  const handleRerollQuestPart = useCallback((part: 'crack' | 'cost_of_inaction' | 'hidden_twist' | 'involvement' | 'antagonist_logic') => {
    setGameState(prev => {
      if (!prev.adventureHook) return prev;
      const quest = { ...prev.adventureHook };
      if (part === 'crack') quest.crack = generateCrack();
      if (part === 'cost_of_inaction') quest.cost_of_inaction = generateCostOfInaction();
      if (part === 'hidden_twist') quest.hidden_twist = generateHiddenTwist();
      if (part === 'involvement' && quest.situation?.involvements && quest.situation.involvements.length > 0) {
        quest.selectedInvolvement = quest.situation.involvements[rollDice(quest.situation.involvements.length) - 1];
      }
      if (part === 'antagonist_logic' && quest.situation?.antagonist_logics && quest.situation.antagonist_logics.length > 0) {
        quest.selectedAntagonistLogic = quest.situation.antagonist_logics[rollDice(quest.situation.antagonist_logics.length) - 1];
      }
      return { ...prev, adventureHook: quest };
    });
    addLog('narrative', `Rerolled quest ${part}.`);
  }, [addLog]);
  ...
  const handleStartAdventure = useCallback(() => {
    ...
    addLog('narrative', `Initial Quest: ${quest.situation?.what || 'A New Adventure Begins'}`);
  }, [addLog]);
  ...
  const handleStartDowntimeActivity = useCallback((activityId: string, variantId: string, customName?: string, goalId?: string, materialCost?: number) => {
    ...
    if (materialCost && gameState.character.gold < materialCost) {
  ...
  }, [addLog, gameState.character.gold]);
  ...
  const handleResolveDowntime = useCallback(() => {
    setGameState(prev => {
      ...
      activity.resolution.negative_modifiers?.forEach((mod: any) => {
        ...
        else if (mod.condition === 'interrupted_more_than_2_days') count = 0;
      ...
      // Add user-added custom manual modifiers
      ad.customModifiers?.forEach(mod => {
        totalScore += mod.value;
      });
      ...
      const logMessage = `Resolved ${ad.name}. Final Score: ${totalScore}. Quality: ${outcome.name}. ${outcome.description}`;
      const newLog = { 
      ...
      const updatedAd: ActiveDowntime = {
        ...ad,
  ...
  const handleAddCustomDowntimeModifier = useCallback((label: string, value: number) => {
    setGameState(prev => {
      if (!prev.activeDowntime) return prev;
      const ad = { ...prev.activeDowntime };
      const customMods = ad.customModifiers || [];
      const newMod = {
        id: Math.random().toString(36).substring(2, 9),
        label,
        value
      };
      ad.customModifiers = [...customMods, newMod];
      return { ...prev, activeDowntime: ad };
    });
  }, []);

  const handleRemoveCustomDowntimeModifier = useCallback((modifierId: string) => {
    setGameState(prev => {
      if (!prev.activeDowntime) return prev;
      const ad = { ...prev.activeDowntime };
      if (!ad.customModifiers) return prev;
      ad.customModifiers = ad.customModifiers.filter(m => m.id !== modifierId);
      return { ...prev, activeDowntime: ad };
    });
  }, []);
